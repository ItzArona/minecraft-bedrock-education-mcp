...
export const CORE_MESSAGES: LocaleMessages = {
  en: {
    ...
    ENCRYPTION_DISABLED: "SocketBE encryption disabled. Use this only for compatibility testing on trusted local networks.",
    WS_DEBUG_ENABLED: "SocketBE WebSocket debug logging enabled.",
    COMMAND_VERSION: (version: number) => `SocketBE command request version override: ${version}`,
    PLAYER_LIST_POLL_DISABLED: "SocketBE automatic player list polling disabled.",
    MINECRAFT_VERSION: (version: string) => `SocketBE command request Minecraft version override: ${version}`,
  ...
  ja: {
    ...
    ENCRYPTION_DISABLED: "SocketBE暗号化は無効です。信頼できるローカル環境での互換性確認にのみ使用してください。",
    WS_DEBUG_ENABLED: "SocketBE WebSocketデバッグログは有効です。",
    COMMAND_VERSION: (version: number) => `SocketBEコマンドリクエストバージョン上書き: ${version}`,
    PLAYER_LIST_POLL_DISABLED: "SocketBEの自動プレイヤーリストポーリングは無効です。",
    MINECRAFT_VERSION: (version: string) => `SocketBEのMinecraftバージョン上書き: ${version}`,
  ...
  zh: {
    ...
    ENCRYPTION_DISABLED: "SocketBE 加密已禁用。请仅在可信本地网络中用于兼容性测试。",
    WS_DEBUG_ENABLED: "SocketBE WebSocket 调试日志已启用。",
    COMMAND_VERSION: (version: number) => `SocketBE 命令请求版本覆盖：${version}`,
    PLAYER_LIST_POLL_DISABLED: "SocketBE 自动玩家列表轮询已禁用。",
    MINECRAFT_VERSION: (version: string) => `SocketBE Minecraft 版本覆盖：${version}`,
